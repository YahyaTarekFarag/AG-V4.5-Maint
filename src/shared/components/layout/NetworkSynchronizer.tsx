...
import { OfflineMediaStore } from '@shared/lib/offlineMedia';
...
export default function NetworkSynchronizer() {
    ...
    const flushQueue = async () => {
        ...
        for (const action of queue) {
            try {
                // Detect and upload offline images 
                const dataToSync = { ...action.data };
                let mediaUploaded = true;

                for (const key in dataToSync) {
                    const val = dataToSync[key];
                    if (typeof val === 'string' && val.startsWith('PENDING_OFFLINE_IMAGE:')) {
                        try {
                            const [fullMarker] = val.split('||');
                            const imageId = fullMarker.split(':')[1];
                            const pendingImages = await OfflineMediaStore.getPending();
                            const media = pendingImages.find(m => m.id === imageId);

                            if (media) {
                                console.log(`📸 Uploading offline image for ${action.table}...`);
                                const url = await uploadToDrive(media.fileData as File, media.folder);
                                dataToSync[key] = url;
                                await OfflineMediaStore.removeImage(imageId);
                            } else if (val.includes('||')) {
                                // Clear corrupted placeholder
                                dataToSync[key] = null;
                            }
                        } catch (uploadErr) {
                            console.error('Failed to sync offline media:', uploadErr);
                            mediaUploaded = false;
                            break;
                        }
                    }
