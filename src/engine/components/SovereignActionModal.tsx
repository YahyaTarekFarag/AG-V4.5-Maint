...
import { applyRBACFilter, getSecureProfileSelect } from '@shared/lib/rbac';
...
export default function SovereignActionModal({ isOpen, onClose, schema, record, onSuccess, isAssignMode, actionTitle }: SovereignActionModalProps) {
    ...
    const form_config = schema?.form_config || { fields: [] };
    const fields = Array.isArray(form_config.fields) ? form_config.fields : [];
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            if (isEdit && record) {
                ...
                const draftKey = `sovereign_draft_${table_name}`;
    ...
    useEffect(() => {
        if (isOpen && !isEdit && Object.keys(formData).length > 0) {
            const draftKey = `sovereign_draft_${table_name}`;
            localStorage.setItem(draftKey, JSON.stringify(formData));
        }
    }, [formData, isOpen, isEdit, table_name]);
    ...
    const fetchLookups = async () => {
        const lookupFields = fields.filter(f => f.type === 'select' && f.dataSource);
    ...
    const handleImageUpload = async (key: string, file: File) => {
        ...
        try {
            ...
            if (!navigator.onLine) {
                // [OFFLINE MODE] Save to IndexedDB and use local preview
                const { OfflineMediaStore } = await import('@shared/lib/offlineMedia');
                const id = await OfflineMediaStore.saveImage(compressed as File, 'maintenance_photos');
                const localUrl = URL.createObjectURL(compressed as Blob);
                setFormData(prev => ({ ...prev, [key]: `PENDING_OFFLINE_IMAGE:${id}||${localUrl}` }));
                showToast('تم حفظ الصورة محلياً وسيتم رفعها عند عودة الشبكة', 'info');
            } else {
    ...
    const hasGpsFields = fields.some(f => isLatField(f) || isLngField(f));
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // ─── Stage 2: Payload Cleaning (Dynamic Whitelist from Schema) ───────────
            // Instead of a hardcoded list, we derive allowed keys from the schema fields
            // plus essential operational columns.
            const operationalWhitelist = [
            ...
            // Proactively include ID columns for relationships if not in fields
            if (schema.directBranchColumn) allowedKeys.add(schema.directBranchColumn);
            if (table_name === 'tickets') {
                ['branch_id', 'category_id', 'assigned_to', 'manager_id', 'brand_id', 'sector_id', 'area_id'].forEach(k => allowedKeys.add(k));
            }

            const finalPayload: Record<string, any> = {};
            Object.keys(processedData).forEach(key => {
                // Only include keys in the allowed set and exclude objects (unless stringified manually)
                const val = processedData[key];
                if (allowedKeys.has(key) && (typeof val !== 'object' || val === null || Array.isArray(val))) {
            ...
            if (isEdit) {
                ...
                if (table_name === 'profiles') {
                    ...
                    delete (updatePayload as any)._password;
                ...
                if (table_name === 'profiles') {
                    ...
                    const password = finalPayload.password || '123456';
                    ...
                    // ─── Operational Excellence: Idempotency ───────────
                    if (!finalPayload.submission_id) {
                        finalPayload.submission_id = `${table_name}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
                    }
            ...
            // Clear draft 
            if (!isEdit) {
                localStorage.removeItem(`sovereign_draft_${table_name}`);
            }
            onSuccess();
        } catch (e: any) {
            const errorMsg = e.message || 'حدث خطأ غير متوقع أثناء معالجة وحفظ البيانات التشغيلية.';
            setError(errorMsg);
    ...
    const title = isAssignMode ? (actionTitle || 'توجيه المأمورية لكادر فني معتمد') : isEdit ? 'تعديل بيانات القيد المعتمدة' : (form_config as any).title || 'تسجيل قيد جديد';
        ...
        <div className="flex flex-col">
                                    ...
                                    <div className="space-y-3">
                                        {formData[field.key] ? (
                                            <div className="relative w-full aspect-video rounded-xl border border-surface-200 overflow-hidden bg-surface-50">
                                                <img
                                                    src={formData[field.key]?.includes('||') ? formData[field.key].split('||')[1] : formData[field.key]}
                                                    alt={field.label}
            ...
            <div className="mt-8 flex justify-end gap-3">
                ...
                >
                    ...
                    {isEdit ? 'حفظ التعديلات المعتمدة' : 'تأكيد التسجيل النهائي'}
                </button>
            </div>
        </div>
    );
