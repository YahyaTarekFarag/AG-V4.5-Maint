...
import SovereignKPICard, { KPICardConfig } from '@/engine/components/SovereignKPICard';
...
export default function Dashboard() {
    ...
    const loadStats = async () => {
        if (!profile) return;
        try {
            // Try V2 RPC first (single optimized query)
            const { data, error } = await supabase.rpc('get_dashboard_stats_v2', { p_profile_id: profile.id });
            ...
            if (!error && data) {
                ...
                setStats({
                    ...
                    availableTechs: statsData?.available_techs || 0,
                    faultyAssets: statsData?.faulty_assets || 0,
                    lowStock: statsData?.low_stock || 0,
                    preventiveDue: statsData?.preventive_due || 0,
                    pendingClosure: (profile.role === 'manager' ? (statsData?.resolved || 0) : 0),
                    loading: false
                });
                return;
            }
            ...
            console.warn('Dashboard RPC unavailable, using direct queries');
            const isGlobal = ['admin', 'maintenance_manager', 'maintenance_supervisor'].includes(profile.role);
            const branchFilter = !isGlobal && profile.branch_id ? { branch_id: profile.branch_id } : {};

            let openQ = supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('status', 'open').eq('is_deleted', false);
            let assignedQ = supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('status', 'assigned').eq('is_deleted', false);
            let inProgressQ = supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('status', 'in_progress').eq('is_deleted', false);
            let resolvedQ = supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('status', 'resolved').eq('is_deleted', false);

            if (branchFilter.branch_id) {
                openQ = openQ.eq('branch_id', branchFilter.branch_id);
                assignedQ = assignedQ.eq('branch_id', branchFilter.branch_id);
                inProgressQ = inProgressQ.eq('branch_id', branchFilter.branch_id);
                resolvedQ = resolvedQ.eq('branch_id', branchFilter.branch_id);
            }

            const [r1, r2, r3, r4] = await Promise.all([openQ, assignedQ, inProgressQ, resolvedQ]);

            setStats({
                open: r1.count || 0,
                assigned: r2.count || 0,
                inProgress: r3.count || 0,
                resolved: r4.count || 0,
                availableTechs: 0,
                faultyAssets: 0,
                lowStock: 0,
                preventiveDue: 0,
                pendingClosure: (profile.role === 'manager' ? (r4.count || 0) : 0),
                loading: false
            });
    ...
    const loadKPICards = async () => {
        try {
            ...
            if (data && data.length > 0) {
                ...
                data.forEach((s: any) => {
                    const configs = s.page_config?.kpi_cards || [];
            ...
            console.error('Error loading KPI cards from schema:', err);
        }

        // Fallback strategy if data is missing, empty, or query failed
        ...
        else if (profile && ['maintenance_manager', 'maintenance_supervisor'].includes(profile.role)) setKpiCards(DEFAULT_MAINT_KPIS);
    ...
    const shortcuts =
        profile?.role === 'manager' ? [
            ...
            profile?.role === 'technician' ? [
                ...
                (profile && ['maintenance_manager', 'maintenance_supervisor'].includes(profile.role)) ? [
        ...
        <DashboardLayout>
            ...
            <div className="mb-8 relative">
                ...
                <p className="text-surface-500 mt-2 text-lg transition-colors font-medium opacity-80 backdrop-blur-sm inline-block">
                    ...
                    {profile?.role === 'maintenance_manager' && 'مركز قيادة الصيانة — الحوكمة التشغيلية، التقييم، وإدارة الموارد الاستراتيجية'}
                    {profile?.role === 'maintenance_supervisor' && 'خلاصة الإشراف الميداني — إدارة التدفقات التشغيلية وحركة القوى الفنية'}
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                ...
                {(profile?.role === 'admin' || profile?.role === 'maintenance_manager' || profile?.role === 'maintenance_supervisor') && stats.lowStock > 0 && (
                    <div
                ...
                {/* Preventive Maintenance Alert KPI */}
                {(profile?.role === 'admin' || profile?.role === 'maintenance_manager' || profile?.role === 'maintenance_supervisor') && stats.preventiveDue > 0 && (
                    <div
                        onClick={() => navigate('/manage/assets')}
                        className="bg-surface-900 p-6 rounded-2xl border border-purple-900/30 shadow-sm relative overflow-hidden group hover:border-purple-400 transition-all cursor-pointer"
                    >
                        <div className="flex items-center gap-4 mb-2 text-purple-400">
                            <AlertTriangle className="w-5 h-5" />
                            <span className="text-sm font-bold">استحقاق صيانة وقائية</span>
                        </div>
                        <div className="text-3xl font-bold text-white">
                            {stats.loading ? <Skeleton variant="rectangular" width="4rem" height="2rem" /> : stats.preventiveDue}
                        </div>
                        <p className="text-xs text-surface-500 mt-1">معدات تجاوزت الفاصل الزمني المخطط للصيانة</p>
                        <div className="absolute top-0 right-0 w-24 h-24 bg-purple-900/10 rounded-full -mr-12 -mt-12 group-hover:scale-110 transition-transform opacity-30" />
                    </div>
                )}
            </div>
