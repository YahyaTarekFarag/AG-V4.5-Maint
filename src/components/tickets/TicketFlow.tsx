    ...
    const handleAction = async (action: 'start_work' | 'resolve' | 'close') => {
        ...
        try {
            ...
            if (action === 'start_work') {
                ...
                // ─── Operational Excellence: Atomic Inventory Deduction ───────────
                if (partsUsed.length > 0) {
                    ...
                    if (invError) throw new Error('خطأ في معالجة صرف قطع الغيار: ' + invError.message);
                }
            ...
            if (updateError) {
                ...
                if (!navigator.onLine || updateError.message?.includes('Fetch')) {
                    ...
                    setError('⚠️ تم حفظ العملية محلياً (وضع عدم الاتصال). سيتم المزامنة آلياً فور عودة الشبكة.');
                    onUpdate();
                    return;
                }
                throw updateError;
                                ...
                                <div key={event.id} className="flex gap-6 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-baseline mb-1">
                                            ...
                                            {event.time && <span className="text-[9px] font-black text-surface-600 uppercase tracking-widest">{format(new Date(event.time), 'hh:mm a', { locale: ar })}</span>}
                                        </div>
