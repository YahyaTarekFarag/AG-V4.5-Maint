-- V27__resolve_ticket_complete_rpc.sql
-- ==========================================
-- توحيد عمليات إغلاق البلاغ (Inventory + Finance + Status)
-- ==========================================

-- التأكد من وجود عمود submission_id في جدول البلاغات للأمان من التكرار
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tickets' AND column_name = 'submission_id') THEN
        ALTER TABLE public.tickets ADD COLUMN submission_id TEXT;
    END IF;
END $$;
...
GRANT EXECUTE ON FUNCTION public.resolve_ticket_complete TO authenticated;
