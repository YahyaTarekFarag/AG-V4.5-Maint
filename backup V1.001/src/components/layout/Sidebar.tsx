...
import { LayoutDashboard, Users, Building2, Package, Wrench, Settings, LogOut, ClipboardList, Map, Timer, ShieldCheck, Smartphone } from 'lucide-react';
...
export default function Sidebar() {
    ...
    const navItems = [
        ...
        { label: 'لوحة الصيانة', path: '/maint-dashboard', icon: Wrench, roles: ['admin', ...MAINT_ROLES] },
        ...
        { label: 'إدارة المخزون', path: '/inventory', icon: Package, roles: ['admin', ...MAINT_ROLES] },
    ...
    return (
        <aside className="w-64 bg-surface-900 text-white flex flex-col min-h-screen fixed right-0 top-0 bottom-0 z-40 transition-transform shadow-2xl">
            ...
            <div className="p-4 border-t border-surface-800 bg-surface-800/10 space-y-2">
                {deferredPrompt && !isInstalled && (
                    <button
                        onClick={handleInstall}
                        className="flex items-center gap-3 w-full px-4 py-3 text-sm font-bold text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl transition-all border border-emerald-500/20"
                    >
